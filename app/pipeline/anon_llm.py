...
Privacy model:
  ...
  2. Those spans are MASKED (replaced with their label tokens) before anything is sent
     to Gemini, so already-identified PII never leaves the system.
  3. Gemini receives only the partially-redacted text.  The unmasked portions are what
     the regex could not catch — that is exactly what we need the LLM to look for.
  4. Few-shot guidance uses label descriptions and per-type counts from
     approved_terms.jsonl, not the actual PII values.
  5. Gemini returns verbatim strings it found in the masked text; we locate those
     strings in the original text to recover exact character positions.
"""
...
# ── Label descriptions sent as few-shot guidance ─────────────────────────────
# These teach the model what each label means without exposing real PII values.
...
The document you will receive has already been PARTIALLY REDACTED: spans that were caught \
by a rule-based system have been replaced with label tokens such as [PATIENT_NAME], [DATE], \
[ADDRESS], etc.  Your job is to find any PII that was MISSED — text that still appears in \
plain form and should have been redacted.
...
- Do NOT report tokens that are already redacted (e.g. do not return "[DATE]" or "[ADDRESS]").
...
def _build_guidance(examples: dict[str, int]) -> str:
    """Build the few-shot guidance block from per-label counts (no raw PII values)."""
    if not examples:
        return ""
    lines = ["From reviewing previous documents of this type, human reviewers have confirmed:\n"]
    for label, count in sorted(examples.items()):
...
def _label_counts() -> dict[str, int]:
    """Count unique (label, text) pairs per label in approved_terms.jsonl."""
    ...
    for line in settings.approved_terms.read_text(encoding="utf-8").splitlines():
        try:
            ...
            counts.setdefault(label, set()).add(text)
        except Exception:
            pass
...
def _mask_text(text: str, entities: list[Entity]) -> str:
    """Apply entity replacements right-to-left so offsets stay valid."""
...
async def run(job: Job) -> Job:
    """Detect missed PII using Gemini on the partially-masked document."""
    if not job.extracted_md:
        return job
    if not settings.google_api_key:
        logger.warning("GOOGLE_API_KEY not set — skipping LLM PII pass")
        return job
    ...
    # ── Privacy: mask already-detected spans before sending to Gemini ─────────
    masked_text = _mask_text(original_text, existing)

    # ── Build prompt — label counts only, no raw PII values ──────────────────
    ...
    audit_log.log(job.id, "llm_anon_done", {
        "sent_to_gemini": "partially_masked_text",
